...
class SumoEnv(gym.Env):
    def __init__(self, net_file, route_file, skip_range, simulation_time, yellow_phase_duration, reward_update_interval, reward_function, mode, use_gui=False):
        ...
        conn = traci
        self.traffic_signal_id = traci.trafficlight.getIDList()[0]
        self.traffic_signal = TrafficSignal(traffic_signal_id=self.traffic_signal_id,
    ...
    def step(self, action):
        ...
        if do_action == -1 and self.change_action_time is None:
            self.change_action_time = self.sumo.simulation.getTime() + self.yellow_phase_duration
    ...
    def _random_skip(self):
        ...
        self.sumo.trafficlight.setRedYellowGreenState(self.traffic_signal.traffic_signal_id, self.traffic_signal.green_phase.state)
        self.traffic_signal.update_end_time()
        self.traffic_signal.reward_update_time = 0
    ...
    def reset(self):
        sumo_cmd = [sumolib.checkBinary(self.sumo_binary), '-n', self.net_file, '-r', self.route_file, '--time-to-teleport', '1000']
        if self.use_gui:
